...
const QuizView = ({ quizData, onNext }) => {
    ...
    return (
        <div className="space-y-4">
            <MathText text={quizData.question} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {Object.entries(quizData.options).map(([key, value]) => (
                    ...
                    >
                       <span className="font-bold mr-2">{key}.</span> <MathText text={value} />
                    </button>
                ))}
            </div>
            ...
            {isRevealed && (
                <div className="mt-4 p-4 bg-gray-900 rounded-lg border border-gray-700">
                    ...
                    <MathText text={quizData.explanation} />
                </div>
            )}
        </div>
    );
...
export default function SyllabusExplorer() {
    ...
    // Unified state for content display
    const [content, setContent] = useState(null);
    const [contentType, setContentType] = useState(null); // 'explanation', 'revise', 'practice'
    ...
    // This function now just sets the topic, and the explanation is fetched by default
    const handleTopicClick = (topic) => {
        setSelectedTopic(topic);
        fetchContent(topic, 'explain'); // Default to explain on first click
    };
