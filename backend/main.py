...
app = FastAPI()
origins = [
    "http://localhost", "http://localhost:8080", "http://localhost:5173", "http://localhost:3000",
...
async def generate_content(request: ContentRequest):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not match_result:
                return JSONResponse(content={
                    "question": None,
                    "error": "Practice questions are not applicable for this introductory topic.", # UPDATED
        ...
        if mode == 'revise':
            ...
            system_message = "..."  # your practice prompt
        else:
            system_message = "..."
        try:
            ...
            if not content or content.lower().startswith("i'm sorry") or content.lower().startswith("i cannot"):
                raise HTTPException(status_code=503, detail="The AI was unable to generate a response for this topic.")
            if mode == 'practice':
                parsed_quiz = parse_quiz_json_from_string(content)
                if parsed_quiz is None:
                    raise HTTPException(status_code=502, detail="The AI returned an invalid format for the quiz question.")
