...
def get_chapter_map_from_db(cursor):
    ...
    cursor.execute("SELECT name, id, class_number FROM chapters")
    return {normalize_name(name): (chapter_id, class_number) for name, chapter_id, class_number in cursor.fetchall()}
